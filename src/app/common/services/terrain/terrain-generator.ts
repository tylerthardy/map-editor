...
export abstract class TerrainGenerator {
  static getRandomAltitudes(width: number, height: number, altitudeMultiplier?: number): number[] {
    ...
    const randomAltitudes: number[] = new Array(numberOfAltitudes).fill(0).map((_) => {
      let randomAltitude: number = Math.random();
      if (altitudeMultiplier) {
        randomAltitude *= altitudeMultiplier;
      }
